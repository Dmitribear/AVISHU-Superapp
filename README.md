...
- `presentation/login_screen.dart`
  Совместимая точка входа для экрана авторизации.

- `presentation/register_screen.dart`
  Совместимая точка входа для экрана регистрации.

- `presentation/auth_access/`
  Atomic-срез для экранов входа и регистрации: shell, бренд-блок, поля ввода, role-card и экранные сценарии.
...
- Единый вход и ролевая маршрутизация: один login-screen и автоматический переход в `client`, `franchisee` или `production` по роли пользователя.
- Клиентская витрина: баннер, каталог, карточка товара, режимы `в наличии` и `предзаказ`, выбор даты готовности, трекинг заказа и программа лояльности.
- Франчайзи: дашборд, поток заказов в реальном времени, принятие заказа, перевод в работу, каталог товаров для управления ассортиментом.
- Производство: упрощённый планшетный интерфейс, крупные карточки задач, короткие статусы, минимум текста, большая кнопка действия на карточке задачи.
- Сквозной сценарий: заказ проходит путь `Клиент -> Франчайзи -> Цех -> Клиент` через Firestore-стримы без ручной перезагрузки экрана.

## Firestore MVP-схема

- `users/{userId}`
  Профиль пользователя, роль, имя, контактные данные, настройки и накопленная лояльность.
- `products/{productId}`
  Карточка товара, цена, галерея, статусы наличия, доступность размеров и параметры предзаказа.
- `orders/{orderId}`
  Центральная сущность заказа: клиент, франчайзи, сумма, статус, доставка, комментарии, расчётная дата готовности.
- `orders/{orderId}/items/{itemId}`
  Позиции заказа: товар, размер, цена, признак предзаказа.
- `orders/{orderId}/history/{entryId}`
  История статусов и действий по заказу для трекинга и аналитики.

## Demo-сценарий для защиты

1. Клиент входит в приложение, выбирает товар и оформляет заказ.
2. Франчайзи видит новый заказ у себя без перезагрузки, принимает его и отправляет в работу.
3. Цех видит задачу в своей очереди, открывает карточку и переводит её в `in production`, затем в `ready`.
4. Клиент получает обновление статуса в своём трекинге и видит, что заказ готов.
...
```text
lib/features/auth/presentation/auth_access/
  atoms/
    auth_footer_strip.dart
    auth_input_field.dart
  molecules/
    auth_brand_block.dart
    auth_role_option_card.dart
  organisms/
    auth_access_shell.dart
  screen/
    login_screen.dart
...
- atoms
  Базовые поля ввода и нижняя сервисная полоска экрана.
- molecules
  Брендовый блок AVISHU и карточка выбора роли.
- organisms
  Общий shell для auth-экранов с рамкой, углами и компоновкой.
- screen
  Отдельные сценарии входа и регистрации без дублирования общего каркаса.
...
```text
lib/features/orders/presentation/production/dashboard_sections/
  atoms/
    production_info_chip.dart
    production_section_label.dart
    production_status_badge.dart
    production_surface_card.dart
  molecules/
    production_analytics_tile.dart
    production_metric_card.dart
    production_task_card.dart
  organisms/
    production_analytics_section.dart
    production_hero_section.dart
    production_order_detail_section.dart
    production_station_section.dart
    production_task_section.dart
  production_dashboard_sections.dart
```

Что здесь за что отвечает:

- atoms
  Базовая подложка, маленькие бейджи и подписи для цехового интерфейса.
- molecules
  Карточки метрик, аналитические тайлы и крупная карточка задачи мастера.
- organisms
  Готовые секции экрана цеха: hero, очередь задач, аналитика, станция и детальная карточка заказа.

Это не “общий UI на все случаи”, а набор секций именно для `ProductionDashboard`.

Что туда вынесено:

- крупный hero-блок для смены цеха
- карточки метрик и фабричная аналитика
- очередь задач и ready-список
- station-экран с инструкцией, потоком и поддержкой
- детальная карточка заказа с главным действием мастера

Что сознательно осталось в `production_dashboard.dart`:

- state экрана
- `TextEditingController`
- вызовы Riverpod-провайдеров
- выбор активной вкладки и активного заказа
- вызовы репозитория для перевода заказа в `inProduction` и `ready`

Это сделано специально: секции уже вынесены, а orchestration экрана осталось в одном месте, чтобы сценарий цеха читался сверху вниз.
...
1. остатки каталогового сценария внутри `client_dashboard.dart`
2. крупные блоки `franchisee_dashboard.dart`, которые ещё живут в одном файле
3. shared order summary-панели, если они начнут повторяться между ролями
4. презентационные материалы под защиту: скриншоты ключевых экранов и короткий demo-script

Смысл именно такой последовательности:

- сначала режем большие сценарии по экранным блокам
- потом смотрим, какие части действительно стали переиспользуемыми
