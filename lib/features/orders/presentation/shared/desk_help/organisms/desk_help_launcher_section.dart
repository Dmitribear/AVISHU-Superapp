...
import 'package:avishu/features/orders/presentation/shared/desk_help/molecules/desk_help_launcher_card.dart';

class DeskHelpLauncherSection extends StatelessWidget {
  final bool compact;
  final String eyebrow;
  final String title;
  final String description;
  final List<DeskHelpLauncherAction> actions;

  const DeskHelpLauncherSection({
    super.key,
        ...
        ...actions.asMap().entries.map(
          (entry) => Padding(
            padding: EdgeInsets.only(
              bottom: entry.key == actions.length - 1 ? 0 : 12,
            ),
            child: DeskHelpLauncherCard(compact: compact, action: entry.value),
          ),
        ),
      ],
    );
